...
class Test_LL_CONNECTION_UPDATE_REQ(unittest.TestCase):
    """ Negative Tests for Link Layer LL_DOS """

    @classmethod
    def setUpClass(cls):
        c = config.loadConfig()
        attack='Connection Update Request'
    ...
    def test_link_layer_LL_CONNECTION_UPDATE_REQ_001(self):
        log.info("Test  CONNECTION_UPDATE_REQ Overflow")
        none_count = 0
        ...
        while True:
            ...
            if data:
                ...
                if LL_FEATURE_RSP in pkt:
                    packet = BTLE(access_addr=self.access_address) / BTLE_DATA() / CtrlPDU() / LL_CONNECTION_UPDATE_REQ(
                        win_size=2,  # 2.5 of windows size (anchor connection window size)
                        win_offset=1,  # 1.25ms windows offset (anchor connection point)
                        interval=20,  # 25ms connection interval
                        latency=3,  # Slave latency (any)
                        timeout=55,  # Supervision timeout, 500ms (any)
                    )

                    packet.len=30 #Overflow
    ...
    def test_link_layer_LL_CONNECTION_UPDATE_REQ_002(self):
        log.info("Test CONNECTION_UPDATE_REQ UNDERFLOW")
