...
class Test_LL_START_ENC_REQ(unittest.TestCase):
    """ Negative Tests for Link Layer LL_START_ENC_REQ """

    @classmethod
    def setUpClass(cls):
        c = config.loadConfig()
        attack='Start Encryption Request'
    ...
    def test_link_layer_LL_START_ENC_REQ_001(self):

        """"Test LL_START_ENC_REQ with length overflow
        Attack Type: CWE-787	Out-of-bounds Write"""
        log.info("Test LL_START_ENC_REQ with length overflow 001")
        ...
        while True:
            ...
            if data:
                ...
                if LL_FEATURE_RSP in pkt:
                    ...
                    pkt = BTLE(access_addr=self.access_address) / BTLE_DATA() / CtrlPDU() / LL_START_ENC_REQ()
                    pkt.len = 48
                    self.driver.send(pkt)
    ...
    #@unittest.skip("skipping test. LL_START_ENC_REQ have length 1, but if we set length to 0 the packet is not transmitted.")
    def test_link_layer_LL_START_ENC_REQ_002(self):
        """"Test LL_START_ENC_REQ with length underflow
        Attack Type: Buffer underflow"""
        log.info("Test LL_START_ENC_REQ with with length underflow 002")
