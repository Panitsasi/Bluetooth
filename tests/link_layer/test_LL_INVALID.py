...
class Test_LL_INVALID(unittest.TestCase):
    """ Negative Tests for Link Layer Invalid Opcode """
    @classmethod
    def setUpClass(cls):
        c = config.loadConfig()
        attack='Invalid'
    ...
    def test_link_layer_LL_INVALID_001(self):
        """"Test with invalid LL opcode"""
        log.info("Test with invalid LL opcode 001")
        ...
        while True:
            ...
            if data:
                ...
                if LL_FEATURE_RSP in pkt:
                    pkt = BTLE(access_addr=self.access_address) / BTLE_DATA() / CtrlPDU() / LL_INVALID()
                    self.driver.send(pkt)
    ...
    def test_link_layer_LL_INVALID_002(self):
        """"Test with invalid LLID=0"""
        log.info("Test with invalid LLID=0 002")
