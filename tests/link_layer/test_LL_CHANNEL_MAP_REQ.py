...
class Test_LL_CHANNEL_MAP_REQ(unittest.TestCase):
    """ Negative Tests for Link Layer LL_CHANNEL_MAP_REQ """

    @classmethod
    def setUpClass(cls):
        c = config.loadConfig()
        attack='Channel Map Request'
    ...
    def test_link_layer_LL_CHANNEL_MAP_REQ_001(self):
        log.info("Test CHANNEL_MAP_REQ OVERFLOW")
        ...
        while True:
            ...
            if data:
                ...
                if LL_FEATURE_RSP in pkt:
                    packet = BTLE(access_addr=self.access_address) / BTLE_DATA() / CtrlPDU() / LL_CHANNEL_MAP_REQ(

                        chM=0x0000000001,
                        #chM=0x1110000000, # Send to advertising channel
                        instant=1000
                    )

                    packet.len=24 #Overflow
    ...
    def test_link_layer_LL_CHANNEL_MAP_REQ_002(self):
        log.info("Test CHANNEL_MAP_REQ UNDERFLOW")
